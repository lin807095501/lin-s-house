...
def test_robust(model, attack_type, c, num_classes, testloader, loss_fn=nn.CrossEntropyLoss(), req_count=10000, is_return=False,nb_iter=10):

    if (attack_type == "pgd"):
        adversary = LinfPGDAttack(
            model, loss_fn=loss_fn, eps=c,
            nb_iter=nb_iter, eps_iter=c/4, rand_init=True, clip_min=0., clip_max=1.,
            targeted=False)
