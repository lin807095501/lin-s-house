...
from networks.resnet import ResNet18_featurealign, ResNet34
from networks.wideresnet import WideResNet, Yao_WideResNet
from networks.mobilenetv2 import MobileNetV2
from networks.vgg import VGG
from AT_helper import adaad_inner_loss_featurealign
...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_featurealign_MSELoss')
...
if args.model == 'mobilenetV2':
    ...
    net = ResNet18_featurealign(num_classes)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD':
            adv_inputs = adaad_inner_loss_featurealign(net, teacher_net, inputs, step_size=args.step_size/255,
            ...
            adv_outputs,s_adv_feature_list = net(adv_inputs,return_fea=True)

            with torch.no_grad():
                teacher_net.eval()
                # t_ori_outputs,t_ori_feature_list = teacher_net(inputs)
                t_adv_outputs,t_adv_feature_list = teacher_net(adv_inputs,return_fea=True)
            ...
            loss_align=0
            for s_f,t_f in zip(s_adv_feature_list,t_adv_feature_list):
                loss_align+=nn.MSELoss()(F.normalize(t_f.detach(),dim=1),F.normalize(s_f.detach(),dim=1))
            loss = args.AdaAD_alpha*kl_loss1 + loss_align
