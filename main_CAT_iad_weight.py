...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_iad_adaptive_epsilon_adjuststudent_weight_1-weight')
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD_IAD':
            ...
            if epoch >= args.IAD_begin:
                loss = args.IAD_alpha*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs/args.temp, dim=1), F.softmax(t_adv_outputs/args.temp, dim=1)).sum(dim=1)).mul(weight)
