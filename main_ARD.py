...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='ARD_ori_alpha08')
    parser.add_argument('--dataset', type=str, default='CIFAR10')
    parser.add_argument('--model', type=str, default='resnet18')
    parser.add_argument('--method', type=str, default='ARD_ori')
    ...
    parser.add_argument('--IAD_alpha', default=0.8, type=float, help='IAD_alpha')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'ARD_SDAD':
            ...
            ori_outputs = net(inputs)
            adv_outputs=net(adv_inputs)
            loss = args.IAD_alpha*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs/args.temp, dim=1), F.softmax(teacher_outputs/args.temp, dim=1)).sum(dim=1))+(1.0-args.IAD_alpha)*nn.CrossEntropyLoss()(ori_outputs, targets)
        else:
            raise NotImplementedError
