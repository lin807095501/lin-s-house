...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='cifar100_adaad_kl_cluster_innerloss')
...
if args.dataset == 'CIFAR10':
    ...
    if args.method not in ['Plain_Clean', 'Plain_Madry']:
        if args.teacher_model == 'Chen2021WRN34_10':
            ...
            teacher_path = '/data/lsb/AdaAD_cat/teacher_ckpt/Chen2021LTD_cifar100.pt'
            teacher_state_dict = torch.load(teacher_path)
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD':
            ...
            if args.dataset == 'CIFAR10':
                ...
                kl_loss1_cluster = (1/len(adv_outputs))*torch.sum(nn.KLDivLoss(reduce=False)(
                    F.log_softmax(adv_outputs.T, dim=1), F.softmax(t_adv_outputs.detach().T, dim=1)))
