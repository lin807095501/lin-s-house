...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_iad_adaptive_epsilon_adjuststudent_1-weight')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD_IAD':
            ...
            weight = (torch.ones(t_adv_outputs.size(0)).cuda() * (epsilons[indices]))
            if epoch >= args.IAD_begin:
                ...
                +args.IAD_beta*args.temp*args.temp*(1/len(num_classes))*torch.sum(KL_loss(F.log_softmax(adv_outputs.T/args.temp, dim=1), F.softmax(t_adv_outputs.detach().T/args.temp, dim=1)).sum(dim=1))
                +args.IAD_alpha*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs, dim=1), F.softmax(ori_outputs, dim=1)).sum(dim=1))
