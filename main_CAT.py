...
from AT_helper import Madry_PGD, adaad_inner_loss_cat
...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_CAT_finetue_inside')
    ...
    parser.add_argument('--eta', type=float, default=0.005, help='perturbation bound')
    ...
    parser.add_argument('--root_path', type=str, default='/data/lsb/AdaAD', help='root path')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD':
            epsilon=epsilon+args.eta
            adv_inputs = adaad_inner_loss_cat(net, teacher_net, inputs, step_size=args.step_size/255,
                                          steps=args.num_steps, epsilon_max=args.epsilon/255,epsilon=epsilon)
            ...
            if torch.argmax(adv_outputs) != targets:
                epsilon-=args.eta
                epsilon=min(args.epsilon/255,epsilon)
            ...
            if args.dataset == 'CIFAR10':
                kl_loss1 = nn.KLDivLoss()(F.log_softmax(adv_outputs, dim=1),
                                          (1-10*epsilon)*F.softmax(t_adv_outputs.detach(), dim=1)+10*epsilon*0.1)
