...
from AT_helper import adaad_inner_loss_cat_labelsmooth_before_softmax,label_smoothing
...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_adaptive_epsilon_labelsmooth_before_softmax_when_generateadv_eta00025')
    ...
    parser.add_argument('--eta', type=float, default=0.0025, help='perturbation eta')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD':
            ...
            adv_inputs = adaad_inner_loss_cat_labelsmooth_before_softmax(net, teacher_net, inputs, step_size=args.step_size/255,
            ...
            if args.dataset == 'CIFAR10':
                kl_loss1 = nn.KLDivLoss()(F.log_softmax(adv_outputs, dim=1),
                                          F.softmax(label_smoothing(t_adv_outputs,epsilons[indices],c=args.c), dim=1))
            ...
            loss = args.AdaAD_alpha*kl_loss1
        else:
            raise NotImplementedError
