...
from AT_helper import ard_cat_loss,ard_ori_loss
...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='IAD_ORI')
    parser.add_argument('--dataset', type=str, default='CIFAR10')
    parser.add_argument('--model', type=str, default='resnet18')
    parser.add_argument('--method', type=str, default='IAD_ORI')
    ...
    parser.add_argument('--IAD_beta', default=1.0, type=float, help='IAD_beta')

    # Inner optimization options
    parser.add_argument('--epsilon_max', type=float, default=16/255, help='perturbation bound')
    parser.add_argument('--init_epsilon', type=int, default=8, help='perturbation bound')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'IAD_SDAD':
            epsilons[indices] += args.eta
            adv_inputs = ard_cat_loss(net,inputs,targets, step_size=args.step_size/255,
            ...
            weight = (torch.ones(t_adv_outputs.size(0)).cuda() * (args.c*epsilons[indices]))#再试一个teacher_loss*（1-weight）+student_loss*weight
            if epoch >= args.IAD_begin:#之后再看需不需要除这个temp kl——loss，试试最后面那里改成weight，而不是1-weight  
                loss = args.IAD_alpha*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs/args.temp, dim=1), F.softmax(t_adv_outputs/args.temp, dim=1)).sum(dim=1))
                # args.IAD_beta*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs.T/args.temp, dim=1), F.softmax(t_adv_outputs.detach().T/args.temp, dim=1)).sum(dim=1))
                +args.IAD_alpha*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs, dim=1), F.softmax(ori_outputs, dim=1)).sum(dim=1).mul(1-weight))
            else:
        ...
        elif args.method == 'IAD_ORI':
            Alpha = torch.ones(len(inputs)).cuda()
            KL_loss = nn.KLDivLoss(reduce=False)
            adv_inputs = ard_ori_loss(net,inputs, targets)
            net.train()
            optimizer.zero_grad()
            ...
            if epoch >= args.IAD_begin:
                for pp in range(len(adv_outputs)):
                    L = F.softmax(teacher_outputs, dim=1)[pp][targets[pp].item()]
                    L = L.pow(args.IAD_beta).item()
                    Alpha[pp] = L
                loss = args.IAD_alpha*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs/args.temp, dim=1),F.softmax(teacher_outputs/args.temp, dim=1)).sum(dim=1)) + args.IAD_alpha*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs, dim=1),F.softmax(net(inputs), dim=1)).sum(dim=1).mul(1-Alpha))+(1.0-args.IAD_alpha)*nn.CrossEntropyLoss()(net(inputs), targets)
            else:
                loss = args.IAD_alpha*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs/args.temp, dim=1),F.softmax(teacher_outputs/args.temp, dim=1)).sum(dim=1))+(1.0-args.IAD_alpha)*nn.CrossEntropyLoss()(net(inputs), targets)
        else:
            raise NotImplementedError
