...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='cifar10_adaad_kl_cluster')
...
def train(epoch):
    ...
    print("Total_Loss:{}  acc:{}".format(train_loss / (batch_idx + 1),Train_acc))
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        correct_PGD10 += torch.max(adv_PGD10_outputs,
                                   1)[1].eq(targets.data).cpu().sum()
        correct_total = total
        print('Total_Loss:{}| Clean Acc: {}| PGD10 Acc: {}'.format(test_loss / (batch_idx + 1),100. * float(correct_ori) / total,100. * float(correct_PGD10) / correct_total))
        # utils.progress_bar(
        #     batch_idx,
        #     len(testloader),
