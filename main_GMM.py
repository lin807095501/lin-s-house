...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_kl_cluster')
        ...
        if args.method == 'AdaAD_IAD':
            ...
            if epoch >= args.IAD_begin:
                for pp in range(len(adv_outputs)):
                    ...
                    Alpha[pp] = L
                loss = args.IAD_alpha*args.temp*args.temp*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs/args.temp, dim=1), F.softmax(t_adv_outputs/args.temp, dim=1)).sum(dim=1)) + args.IAD_alpha*(
                    1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs, dim=1), F.softmax(net(inputs), dim=1)).sum(dim=1).mul(1-Alpha))
            else:
