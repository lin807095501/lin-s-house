...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_iad_adaptive_epsilon_tanh_weight')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD_IAD':
            ...
            weight = (torch.ones(t_adv_outputs.size(0)).cuda() * (torch.tanh(args.c*epsilons[indices])/ torch.tanh(args.c*torch.tensor(args.epsilon_max))).cuda())#再试一个teacher_loss*（1-weight）+student_loss*weight
