...
from AT_helper import adaad_inner_loss_cat_nols,label_smoothing_nodirichlet
...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_onlyep_nols_normalattack')
    ...
    parser.add_argument('--epsilon_max', type=float, default=8/255, help='perturbation bound')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD':
            ...
            adv_inputs = adaad_inner_loss_cat_nols(net, teacher_net, inputs, step_size=args.step_size/255,
            ...
            if args.dataset == 'CIFAR10':
                kl_loss1 = nn.KLDivLoss()(F.log_softmax(adv_outputs, dim=1),
                                          F.softmax(t_adv_outputs, dim=1))
                # kl_loss1 = nn.KLDivLoss()(F.log_softmax(adv_outputs, dim=1),
                #                           label_smoothing_nodirichlet(F.softmax(t_adv_outputs, dim=1),epsilons[indices],c=args.c))
