...
def get_args():
    ...
    parser.add_argument('--name', type=str, default='adaad_sdad_gaussian_version2_weight')
...
def get_weight(args,epsilons,beta=-1,alpha=1):
    weight=args.epsilon_max*10/2.0*(1-torch.erf((beta-alpha)*epsilons/args.epsilon_max+alpha))
    return weight
# Training
def train(epoch):
    print('\nEpoch: %d' % epoch)
    ...
    for batch_idx, (inputs, targets,indices) in enumerate(trainloader):
        ...
        if args.method == 'AdaAD_IAD':
            ...
            weight = (torch.ones(t_adv_outputs.size(0)).cuda() * get_weight(args,epsilons[indices]))
            if epoch >= args.IAD_begin:
                ...
                +args.IAD_alpha*(1/len(adv_outputs))*torch.sum(KL_loss(F.log_softmax(adv_outputs, dim=1), F.softmax(ori_outputs, dim=1)).sum(dim=1).mul(weight))
